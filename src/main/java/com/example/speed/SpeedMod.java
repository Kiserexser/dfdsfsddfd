...
public class SpeedMod implements ModInitializer {
    ...
    public void onInitialize() {
        LOGGER.info("WebFly loaded. Press G to toggle.");
        ...
        workerThread = new Thread(() -> {
            while (running) {
                try {
                    if (mc != null && mc.getWindow() != null) {
                        ...
                        if (gPressed && !wasGPressed) {
                            enabled = !enabled;
                            LOGGER.info("WebFly: " + (enabled ? "ON" : "OFF"));
                            wasGPressed = true;
                        } else if (!gPressed) {
                            wasGPressed = false;
                        }
                ...
                catch (Exception e) { LOGGER.error("WebFly error", e); }
            }
        });
    ...
    private void onTick() {
        ...
        if (!isInWebOrBerriesOrCampfire()) return;
    ...
    private boolean isInWebOrBerriesOrCampfire() {
        BlockPos pos = mc.player.getBlockPos();
        var state = mc.world.getBlockState(pos);
        var stateUp = mc.world.getBlockState(pos.up());
        var stateDown = mc.world.getBlockState(pos.down()); // на случай, если игрок стоит на блоке
        ...
        return state.isOf(Blocks.COBWEB) || stateUp.isOf(Blocks.COBWEB) || stateDown.isOf(Blocks.COBWEB) ||
               ...
               state.isOf(Blocks.CAMPFIRE) || stateUp.isOf(Blocks.CAMPFIRE) || stateDown.isOf(Blocks.CAMPFIRE) ||
               state.isOf(Blocks.SOUL_CAMPFIRE) || stateUp.isOf(Blocks.SOUL_CAMPFIRE) || stateDown.isOf(Blocks.SOUL_CAMPFIRE);
    }
}
